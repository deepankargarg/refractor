...
public class StatementPrinter {
    ...
    public String statement() {
        ...
        for (Performance p : getInvoice().getPerformances()) {
            ...
            switch (play.getType()) {
                case "tragedy":
                    ...
                    if (p.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                        ...
                        thisAmount += Constants.HISTORY_OVER_BASE_CAPACITY_PER_PERSON * (p.getAudience() - x);
                    }
                    break;
                case "comedy":
            ...
            final int y = thisAmount / Constants.PERCENT_FACTOR;
            result.append(String.format("  %s: %s (%s seats)%n", play.getName(), frmt.format(y), p.getAudience()));
