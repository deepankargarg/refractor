...
public class StatementPrinter {
    ...
    public String statement() {
        ...
        for (Performance p : getInvoice().getPerformances()) {
            ...
            final int y = getAmount(p) / Constants.PERCENT_FACTOR;
            final Play x = getPlay(p);
            result.append(String.format("  %s: %s (%s seats)%n", x.getName(), frmt.format(y), p.getAudience()));
    ...
    protected static Play getPlay(Performance performance) {
        ...
        switch (getPlay(performance).getType()) {
            case "tragedy":
                ...
                if (performance.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                    final int x = Constants.BASE_VOLUME_CREDIT_THRESHOLD;
                    result += Constants.HISTORY_OVER_BASE_CAPACITY_PER_PERSON * (performance.getAudience() - x);
                }
                break;
            case "comedy":
